...
Usage:
    python scripts/gen_ko_pdf.py [repo_root] [out_docx]
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(REPO, "DeepSeek-Harness-\ud575\ub4dc\ubd81.docx")

LATIN = "Calibri"
MONO = "Consolas"
# Korean-capable CJK font actually installed on the machine doing the DOCX->PDF
# conversion (macOS system font). Substitute with "Malgun Gothic" if rendering
# on Windows.
CJK = "Apple SD Gothic Neo"
...
NEXT_CHAPTER_MARKERS = ("**\ub2e4\uc74c \uc7a5**",)  # **다음 장**
...
def clean_md_lines(raw_lines):
    ...
    if lines and lines[0].lstrip().startswith("[English]"):
        lines = lines[1:]
    # drop '---' + '**다음 장**: ...' footer (chapter-to-chapter web navigation)
    out = []
    for i, l in enumerate(lines):
        stripped = l.strip()
        if stripped == "---" and i + 1 < len(lines) and lines[i + 1].strip().startswith(NEXT_CHAPTER_MARKERS):
            continue
        if stripped.startswith(NEXT_CHAPTER_MARKERS):
            continue
        if re.match(r"^\s*<!--.*-->\s*$", l):
            continue
        out.append(l)
...
def build_cover(doc):
    ...
    add_inline(p, "DeepSeek Harness \ud578\ub4dc\ubd81", size=30, bold=True, color=NAVY)  # 핸드북

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_after = Pt(18)
    add_inline(p, "dsh-handbook \u00b7 \ud55c\uad6d\uc5b4 \uc644\uc5ed\ud310", size=16, bold=True)  # 한국어 완역판

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_after = Pt(6)
    add_inline(p, "DeepSeek\uac00 \uacf5\uc2dd \uc624\ud508\uc18c\uc2a4\ub85c \uacf5\uac1c\ud55c Agent \ub7f0\ud0c0\uc784\uc744 0\ubd80\ud130 1\uae4c\uc9c0 \u2014\u2014 "
                  "\ucd08\ubcf4\uc790\ub97c \uc704\ud55c \ubc31\uacfc\uc0ac\uc804: \uc124\uce58, \uc0ac\uc6a9, \ud50c\ub7ec\uadf8\uc778 \uac1c\ubc1c, "
                  "\ud29c\ub2dd, \ubcf4\uc548, \uc608\uc0b0\uae4c\uc9c0.",
    ...
    add_inline(p, "16\uac1c \ucc55\ud130 + \ubd80\ub85d(\uc6a9\uc5b4\uc9d1 / \ud328\ud0a4\uc9c0 / Benchmark)", size=12, bold=True)  # 16개 챕터 + 부록(용어집 / 패키지 / Benchmark)

    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_inline(p, "docs/\uc5d0\uc11c \ub3d9\uae30\ud654 \u00b7 dsh 0.1.0-rc.6 \u00b7 2026-08-17", size=10, color=GRAY)  # docs/에서 동기화
...
def build_toc(doc, chapters, appendices):
    render_heading(doc, 1, "\ubaa9\ucc28", page_break=False)  # 목차
